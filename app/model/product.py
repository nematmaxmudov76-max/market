...
if TYPE_CHECKING:
    ...
    from .user import User, Like, User_Rating, User_Search
...
class Shop(BaseMain):
    ...
    user: Mapped["User"] = relationship(
        "User", back_populates="shop", lazy="raise_on_sql"
    )
    media: Mapped[list["Media"]] = relationship(
        "Media", back_populates="shop", lazy="raise_on_sql"
    )
...
class Media(BaseMain):
    ...
    product_media: Mapped[list["Product_Media"]] = relationship(
        "Product_Media", back_populates="media", lazy="raise_on_sql"
    )
    user: Mapped["User"] = relationship(
        "User", back_populates="media", lazy="raise_on_sql"
    )
...
class Product(BaseMain):
    ...
    order_item: Mapped[list["Order_Item"]] = relationship(
        "Order_Item", back_populates="product", lazy="raise_on_sql"
    )
    bucket_product: Mapped[list["Bucket_Product"]] = relationship(
        "Bucket_Product", back_populates="product", lazy="raise_on_sql"
    )
    product_media: Mapped[list["Product_Media"]] = relationship(
        "Product_Media", back_populates="product", lazy="raise_on_sql"
    )
    category: Mapped["Category"] = relationship(
        "Category", back_populates="product", lazy="raise_on_sql"
    )
    comment: Mapped[list["Comment"]] = relationship(
        "Comment", back_populates="product", lazy="raise_on_sql"
    )
    shop_product: Mapped[list["Shop_Product"]] = relationship(
        "Shop_Product", back_populates="product", lazy="raise_on_sql"
    )
    ...
    user_rating: Mapped[list["User_Rating"]] = relationship(
        "User_Rating", back_populates="product", lazy="raise_on_sql"
    )
    user_search: Mapped[list["User_Search"]] = relationship(
        "User_Search", back_populates="product", lazy="raise_on_sql"
    )
...
class Product_Media(BaseMain):
    ...
    product: Mapped["Product"] = relationship(
        "Product", back_populates="product_media", lazy="raise_on_sql"
    )
    media: Mapped["Media"] = relationship(
        "Media", back_populates="product_media", lazy="raise_on_sql"
    )
...
class Shop_Product(BaseMain):
    ...
    shop: Mapped["Shop"] = relationship(
        "Shop", back_populates="shop_product", lazy="raise_on_sql"
    )
    product: Mapped["Product"] = relationship(
        "Product", back_populates="shop_product", lazy="raise_on_sql"
    )
...
class Comment(BaseMain):
    ...
    deleted_comment: Mapped[str] = mapped_column(
        Text, default=None
    )  # deleted comment is => deleted_comment

    def __repr__(self):
        return (
            f"user id:{self.user_id}, product id:{self.product_id}, title:{self.title}"
        )

    user: Mapped["User"] = relationship(
        "User", back_populates="comment", lazy="raise_on_sql"
    )
    product: Mapped["Product"] = relationship(
        "Product", back_populates="comment", lazy="raise_on_sql"
    )
