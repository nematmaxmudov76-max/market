...
class Payment_Process(BaseMain):
    ...
    user: Mapped["User"] = relationship(
        "User", back_populates="payment_process", lazy="raise_on_sql"
    )
    order: Mapped["Order"] = relationship(
        "Order", back_populates="payment_process", lazy="raise_on_sql"
    )
    wallet: Mapped["Wallet"] = relationship(
        "Wallet", back_populates="payment_process", lazy="raise_on_sql"
    )
    transaction_log: Mapped[list["Transaction_Log"]] = relationship(
        "Transaction_Log", back_populates="payment_process", lazy="raise_on_sql"
    )
...
class Transaction_Log(BaseMain):
    ...
    payment_process: Mapped["Payment_Process"] = relationship(
        "Payment_Process", back_populates="transaction_log", lazy="raise_on_sql"
    )
    wallet: Mapped["Wallet"] = relationship(
        "Wallet", back_populates="transaction_log", lazy="raise_on_sql"
    )
